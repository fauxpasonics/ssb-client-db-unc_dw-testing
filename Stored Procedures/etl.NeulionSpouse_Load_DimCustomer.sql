...
CREATE PROCEDURE [etl].[NeulionSpouse_Load_DimCustomer]

AS
BEGIN

--Delete merged records from DimCustomer
DELETE
FROM dbo.DimCustomer WHERE SourceSystem = 'NeulionSpouse' AND ssid IN (SELECT DISTINCT [Member ID] FROM SSB_BIMDM.ods.SaasMerge)
...
SELECT * 
INTO #tmpETL 
FROM etl.[vw_NeulionSpouse_Load_DimCustomer]
...
THEN UPDATE SET 
	myTarget.SourceDB = mySource.SourceDB
	, myTarget.SourceSystem = LEFT(mySource.SourceSystem,50)
	, myTarget.SSID = LEFT(mySource.SSID,100)
	, myTarget.AddressPrimaryDirtyHash = mySource.AddressPrimaryDirtyHash
	, myTarget.AddressPrimaryIsCleanStatus = 'Dirty'
	, myTarget.AddressPrimaryStreet = LEFT(mySource.AddressPrimaryStreet,500)
	, myTarget.AddressPrimaryCity = LEFT(mySource.AddressPrimaryCity,200)
	, myTarget.AddressPrimaryState = LEFT(mySource.AddressPrimaryState,200)
	, myTarget.AddressPrimaryZip = LEFT(mySource.AddressPrimaryZip,25)
	, myTarget.AddressPrimaryCounty = LEFT(mySource.AddressPrimaryCounty,200)
	, myTarget.AddressPrimaryCountry = LEFT(mySource.AddressPrimaryCountry,200)
...
THEN UPDATE SET 
	myTarget.CustomerType = mySource.CustomerType
	, myTarget.CustomerStatus = LEFT(mySource.CustomerStatus,50)
	, myTarget.AccountType = LEFT(mySource.AccountType,50)
	, myTarget.AccountRep = LEFT(mySource.AccountRep,50)
	, myTarget.CompanyName = LEFT(mySource.CompanyName,500)
	, myTarget.SalutationName = LEFT(mySource.SalutationName,50)
	, myTarget.DonorMailName = LEFT(mySource.DonorMailName,50)
	, myTarget.DonorFormalName = LEFT(mySource.DonorFormalName,50)
	, myTarget.Birthday = mySource.Birthday
	, myTarget.Gender = LEFT(mySource.Gender,10)
	, myTarget.AccountId = mySource.AccountId
	, myTarget.MergedRecordFlag = mySource.MergedRecordFlag
	, myTarget.MergedIntoSSID =  LEFT(mySource.MergedIntoSSID,100)
	, myTarget.IsBusiness = mySource.IsBusiness
	, myTarget.customer_matchkey = LEFT(mySource.customer_matchkey,50)
	, myTarget.ExtAttribute1 = LEFT(mySource.ExtAttribute1,100)
	...
	, myTarget.SSCreatedBy = LEFT(mySource.SSCreatedBy,255)
	, myTarget.SSUpdatedBy = LEFT(mySource.SSUpdatedBy,255)
	, myTarget.SSCreatedDate = mySource.SSCreatedDate
	, myTarget.SSUpdatedDate = mySource.SSUpdatedDate
	, myTarget.UpdatedBy = 'CI'
    , myTarget.UpdatedDate = @RunTime
