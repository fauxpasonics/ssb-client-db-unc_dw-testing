...
SET ANSI_NULLS ON
GO
create procedure [sascdm].[usp_Em_Responses] as 
/*
...
The data from the above staging tables is loaded to the EM_RESPONSE_TABLE/EM_SEND_URL and EMXREF tables.
...
*/
set nocount on;
declare @schema as nvarchar(12) = 'sascdm';
...
begin try
	set @sqltext = 'merge into ' + @em_table + ' as perm ' +
					...
					'   when not matched then '  +
					'        insert ( ' + @response_history_columns  + ') ' +
					'         values( ' + @stg_columns_for_insert +  ' null, null, null); '
	exec sp_executesql @sqltext
	set @rowkount = @@ROWCOUNT
end try
begin catch
...
begin try
	set @sqltext = 'merge into ' + @em_table + ' as perm ' +
					...
					'   when not matched then '  +
					'        insert ( ' + @response_history_columns  + ') ' +
					'         values( ' + @stg_columns_for_insert +  ' null, stg.unsubreason, null); '
	exec sp_executesql @sqltext
	set @rowkount = @@ROWCOUNT
end try
begin catch
...
begin try
	set @sqltext = 'merge into ' + @em_table + ' as perm ' +
					...
					'   when not matched then '  +
					'        insert ( ' + @response_history_columns  + ') ' +
					'         values( ' + @stg_columns_for_insert +  ' null, stg.bouncereason, stg.bouncecategory); '
	exec sp_executesql @sqltext
	set @rowkount = @@ROWCOUNT
end try
begin catch
...
begin try
	set @sqltext = 'merge into ' + @em_table + ' as perm ' +
					...
					'   when not matched then '  +
					'        insert ( ' + @response_history_columns  + ') ' +
					'         values( ' + @stg_columns_for_insert +  ' null, stg.reason, null); '
	exec sp_executesql @sqltext
	set @rowkount = @@ROWCOUNT
end try
begin catch
