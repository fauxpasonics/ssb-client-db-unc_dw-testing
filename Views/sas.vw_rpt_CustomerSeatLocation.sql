...
CREATE VIEW [sas].[vw_rpt_CustomerSeatLocation]
AS
...
JOIN dbo.TK_SEAT_SEAT ss (NOLOCK)
	ON t.SEASON = ss.SEASON and t.CUSTOMER = ss.CUSTOMER
	AND t.SEQ = ss.SEQ AND toec.EVENT = ss.EVENT
...
JOIN dbo.TK_EVENT te (NOLOCK)
	ON te.EVENT = toec.[EVENT]
	AND te.SEASON = toec.SEASON
