...
CREATE VIEW [sas].[vw_rpt_ConstituentSummary]
AS
...
LEFT JOIN dbo.DimTicketCustomer dtc (NOLOCK)
	ON dc.SourceSystem = dtc.ETL__SourceSystem AND dc.SSID = dtc.ETL__SSID
LEFT JOIN FirstStep fts (NOLOCK)
	ON dtc.DimTicketCustomerId = fts.DimTicketCustomerId
LEFT JOIN SecondStep bc (NOLOCK)
	ON bc.CUSTOMER = dc.SSID AND dc.SourceSystem = 'PAC'
LEFT JOIN sas.vw_Neulion_Contrib nc (NOLOCK)
	ON dc.ssid = CAST(nc.MemberID AS NVARCHAR(100)) AND dc.SourceSystem <> 'Pac'
LEFT JOIN ods.BB_RevenueSummary bb (NOLOCK)
	ON dc.SSID = CAST(bb.CONSTITUENTID AS NVARCHAR(100))
