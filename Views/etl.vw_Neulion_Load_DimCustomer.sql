...
CREATE VIEW [etl].[vw_Neulion_Load_DimCustomer] AS 
...
*/
		...
		SELECT 
			...
			, a.Salutation AS [SalutationName]
			, a.[Name] AS [DonorMailName]
			, a.[Name] AS [DonorFormalName]
			, TRY_CAST(a.[BirthDate] AS DATE) AS [Birthday]
			, NULL AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/**ENTITIES**/
			/*Name*/			
			, LEFT(a.[Name],200) AS FullName
			, LEFT(NULL,100) AS [Prefix]
		
			, a.[LastName] AS [LastName]
			, a.[MiddleInit] AS [MiddleName]
			, a.[FirstName] AS  [FirstName]
		
			, LEFT(a.Suffix,100) AS [Suffix]
			...
			, TRY_CAST(a.[BirthDate] AS DATE) AS [ExtAttribute21] --datetime
			...
			, GETDATE() [CreatedDate]
			, 0 AS IsDeleted
			, TRY_CAST([MostRecentContactChangeDate] AS DATE) [SSUpdatedDate]

			, NULL [AccountId]
			, 0 IsBusiness
			, CASE WHEN a.DavieID <> '99999999' THEN a.DavieID
...
--		SELECT top 100 *
		...
		AND a.[MemberID] NOT IN (SELECT DISTINCT [Member ID] FROM  src.SaasMerge)
