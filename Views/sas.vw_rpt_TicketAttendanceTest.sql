...
CREATE VIEW [sas].[vw_rpt_TicketAttendanceTest]
AS
...
JOIN dbo.TK_ODET_EVENT_ASSOC toec (NOLOCK) ON t.season = toec.SEASON AND t.CUSTOMER = toec.CUSTOMER AND t.SEQ = toec.SEQ
JOIN dbo.TK_SEAT_SEAT ss (NOLOCK) ON t.CUSTOMER = ss.CUSTOMER AND t.SEQ = ss.SEQ AND toec.EVENT = ss.EVENT
JOIN dbo.TK_BC bc (NOLOCK) ON bc.BC_ID = ss.BARCODE
JOIN dbo.TK_SEASON ts (NOLOCK) ON t.SEASON = ts.SEASON
JOIN dbo.TK_EVENT te (NOLOCK) ON t.ITEM = te.[EVENT] AND t.SEASON = te.SEASON
JOIN dbo.TK_ITEM ti (NOLOCK) ON t.ITEM = ti.ITEM
JOIN dbo.TK_PRTYPE pt (NOLOCK) ON t.I_PT = pt.PRTYPE
JOIN dbo.vwDimCustomer_ModAcctId dc (NOLOCK) ON t.CUSTOMER = dc.SSID
JOIN mdm.CompositeRecord cr (NOLOCK) ON dc.SSB_CRMSYSTEM_CONTACT_ID = cr.SSB_CRMSYSTEM_CONTACT_ID
JOIN dbo.TicketTagging tg (NOLOCK) ON t.Season COLLATE SQL_Latin1_General_CP1_CI_AS = tg.Season
